...
echo "== [2/12] brew formulae"
brew install fd gh git-lfs go-task pnpm uv awscli ripgrep htop tree docker || note_fail "brew formulae"

echo "== [3/12] brew casks (CLI tools + MonitorControl)"
...
echo "== [7/12] config files (home/ → \$HOME)"
...
echo "== [8/12] script clones (~/scripts)"
clone_or_pull() {
    local url=$1 dest=$2
    if [ -d "$dest/.git" ]; then
